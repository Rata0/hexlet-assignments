...
public class ProductsController {
    ...
    public ProductDTO update(@PathVariable Long id, @RequestBody ProductUpdateDTO postDto) {
        var product = productRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Not found"));
        updateToEntity(postDto, product);
        productRepository.save(product);
        var productDto = toDTO(product);
        return productDto;
    }

    private Product updateToEntity(ProductUpdateDTO postDto, Product product) {
